(() => {
  ...
  function isCuratedSbtSource(source) {
    return sourceId(source) === "website-sbt-system-summary";
  }
  ...
  function aggregatorCardLink(source, card) {
    if (isCuratedSbtSource(source)) return "./index.html#cat-sbt";
  ...
  function formatProviderName(value) {
    ...
    if (raw.toLowerCase() === "curated") return "SBT Matrix";
    return raw;
  }
  ...
  async function loadAgentFeedCard(source) {
    ...
    if (!id) return null;
    if (isCuratedSbtSource(source)) return null;
    const lang = getUiLangTag();
  ...
  function markdownTableHtml(lines) {
    ...
    return `
      <div class="agent-answer-table-wrap">
        <table class="agent-answer-table">
          <thead>
            <tr>${header.map((cell) => `<th>${inlineMessageHtml(cell)}</th>`).join("")}</tr>
          </thead>
          <tbody>
            ${bodyRows.map((row) => `
              <tr>${header.map((_, idx) => `<td>${inlineMessageHtml(row[idx] || "")}</td>`).join("")}</tr>
            `).join("")}
          </tbody>
        </table>
      </div>
    ...
    return blocks
      .map((part) => {
        ...
        if (lines.length >= 2 && /^\|/.test(lines[0]) && /^\|?\s*:?-{3,}:?/.test(lines[1])) {
          const table = markdownTableHtml(lines);
          if (table) return table;
        }
        if (lines.length === 1 && /^#{1,4}\s+/.test(lines[0])) {
          const title = lines[0].replace(/^#{1,4}\s+/, "");
          return `<h3>${inlineMessageHtml(title)}</h3>`;
        }
        if (lines.length === 1 && /^\*\*[^*]+\*\*$/.test(lines[0])) {
          return `<h3>${inlineMessageHtml(lines[0].replace(/^\*\*|\*\*$/g, ""))}</h3>`;
        }
        if (lines.length && lines.every((line) => /^>\s?/.test(line))) {
          return `<aside class="agent-answer-note">${lines.map((line) => inlineMessageHtml(line.replace(/^>\s?/, ""))).join("<br>")}</aside>`;
        }
        if (lines.length && lines.every((line) => /^[-*]\s+/.test(line))) {
  ...
  function renderOriginalCardPreview(card, source, mode = "loading") {
    if (isCuratedSbtSource(source)) {
      return `
        <section class="agent-original-card is-sbt-source">
          <div class="agent-original-card-kicker">Website SBT Matrix</div>
          <h3>SBT 系統整理（近期）</h3>
          <p>這不是從單篇貼文猜答案，而是直接使用聚合器 SBT 欄位整理好的可取得清單、條件與難度。</p>
          <div class="agent-original-card-actions">
            <a href="./index.html#cat-sbt"><iconify-icon icon="lucide:panel-top-open"></iconify-icon><span>在聚合器查看 SBT 欄位</span></a>
          </div>
        </section>
      `;
    }
  ...
  async function askRenaissAgent(question) {
    ...
    try {
      ...
      const sourceLabel = provider === "SBT Matrix" ? (count === 1 ? "curated source" : "curated sources") : "memory sources";
      setRenaissAgentStatus(`${provider} · ${count} ${sourceLabel}`, "ok");
    } catch (error) {
      removeTypingBubble();
